<?php
    ...
    if (empty($cur_product_id) or empty(findProductById($cur_product_id, $products))){
        header('Location: product_gallery.php');
        die();
    }
    ...
    if (isset($_POST["product_name"], $_POST["category_list"], $_POST["product_price"], $_POST["color_picker_1"], $_POST["color_picker_2"], $_POST["color_picker_3"], $_POST["product_text"], $_FILES["product_picture"], $_SESSION["csfr_token"], $_POST["csfr_token"]) and $_POST["csfr_token"] == $_SESSION["csfr_token"]){
        ...
        if (!empty(findProductByName($product_name, loadProductsExceptId($cur_product_id)))){
        ...
        if (count($product_problems) == 0){
            ...
            $picture_path = "./product_pictures/" . $cur_product_id . "." . $picture_file_type;
            ...
            $filtered_products = loadProductsExceptId($cur_product_id);

            if (empty($filtered_products)){
                $filtered_products = array();
            }
            array_push($filtered_products, $new_product);
...
<main id="reg_main">
    <div class="reg-form">
        <form class="reg-table" method="post" action="edit_product.php" enctype="multipart/form-data" onsubmit="return validate_add_product();">
            <div class="reg-title">
                <h1>Editace produktu</h1>
            </div>

            <div class="reg_label"><label for="product_name">Jméno*</label><br></div>
            <div class="reg_field"><input type="text" class="<?php if ($name_problem){echo "product_name_red";}?>" id="product_name" name="product_name" required value="<?php if ($first_visit){echo htmlspecialchars($product['name']);} else if(isset($_POST['product_name'])){ echo htmlspecialchars($_POST['product_name']);}?>"><br></div>
            ...
            <select name="category_list" id="category_list" size="1" class="add_product_category_select">
                ...
                if (!empty($categories)){
                    foreach ($categories as $cur_category){
                        ...
                        if ($first_visit and $cur_category_name == $category["name"]){
                            echo "
                        ...
                        else if (!$first_visit and $cur_category_name == $category_list){
                            echo "
                                <option value=\"$cur_category_name\" selected>$cur_category_name</option>    
                            ";
                            continue;
            ...
            <div class="reg_field"><input type="number" min="100" max="100000" class="<?php if ($product_price_problem){echo "product_price_red";}?>" id="product_price" name="product_price" value="<?php if ($first_visit){echo htmlspecialchars($product['product_price']);} else if(isset($_POST['product_price'])){ echo htmlspecialchars($_POST['product_price']);}?>" required><br></div>

            <div class="reg_label"><label for="color_picker_1">Barva 1*</label><br></div>
            <div class="reg_field"><input type="color" id="color_picker_1" name="color_picker_1" value="<?php if ($first_visit){echo htmlspecialchars($product['color_1']);} else if(isset($_POST['color_picker_1'])){ echo htmlspecialchars($_POST['color_picker_1']);}?>"><br></div>

            <div class="reg_label"><label for="color_picker_2">Barva 2*</label><br></div>
            <div class="reg_field"><input type="color" id="color_picker_2" name="color_picker_2" value="<?php if ($first_visit){echo htmlspecialchars($product['color_2']);} else if(isset($_POST['color_picker_2'])){ echo htmlspecialchars($_POST['color_picker_2']);}?>"><br></div>

            <div class="reg_label"><label for="color_picker_3">Barva 3*</label><br></div>
            <div class="reg_field"><input type="color" id="color_picker_3" name="color_picker_3" value="<?php if ($first_visit){echo htmlspecialchars($product['color_3']);} else if(isset($_POST['color_picker_3'])){ echo htmlspecialchars($_POST['color_picker_3']);}?>"><br></div>

            <div class="reg_label"><label for="product_text">Popis*</label><br></div>
            <div class="reg_field"><textarea class="<?php if ($product_text_problem){echo "product_text_red";}?>" id="product_text" name="product_text" rows="3" cols="50" required><?php if ($first_visit){echo htmlspecialchars($product['product_text']);} else if(isset($_POST['product_text'])){ echo htmlspecialchars($_POST['product_text']);}?></textarea><br></div>

            <input type="hidden" name="csfr_token" value="<?php echo $_SESSION['csfr_token'];?>">

            <label for="edit_submit"></label>
            <div class="reg_submit"><input type="submit" name="edit_submit" id="edit_submit" value="Přidat produkt"></div>
