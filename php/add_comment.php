<?php
    ...
    if (!(isset($_SESSION['user'])) or $_SESSION['user']['isAdmin'] or !$_SESSION['user']['isApproved']){
    ...
    if (isset($_POST['comment_header'], $_POST['comment_text'], $_SESSION["visiting_product_id"])){
        ...
        if (count($problems_arr) == 0){
            ...
            $new_comment = array(
                "id" => findCommentMaxIndex($all_comments) + 1,
                "author_email" => $_SESSION['user']['email'],
                "product_id" => intval($_SESSION['visiting_product_id']),
            ...
            array_push($all_comments, $new_comment);
...
<main id="log_main">
    <div class="add-comment-table">
        <form class="add-comment-form" action="add_comment.php" method="post" onsubmit="return validate_discussion();">
            <h1 class="add_comment_title">Přidání diskuzního příspěvku</h1>
            ...
            <div class="comment_submit"><input type="submit" id="cm_submit" name="cm_submit" value="Odeslat příspěvek"></div>
        </form>
    </div>
